...
# We haven't discuss linspace yet but for this lab, it is sufficient for you to know what the contents of the numpy array months are
...
max_month = np.max(data_sea_ice, axis = 1)
max_col = data_sea_ice * (data_sea_ice ==  np.reshape(max_month, (35, 1)))
max_row = np.sum(max_col, axis = 0)
max_total = months[np.argmax(max_row)]
